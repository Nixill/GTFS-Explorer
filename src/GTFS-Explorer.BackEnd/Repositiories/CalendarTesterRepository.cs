...
namespace GTFS_Explorer.BackEnd.Repositiories
{
    public class CalendarTesterRepository : ICalendarTesterRepository
    {
        ...
        {
            Calendar cal = _reader.Feed.Calendars.Get(serviceId).FirstOrDefault();
            IEnumerable<CalendarDate> calDates = _reader.Feed.CalendarDates.Where(x => x.ServiceId == serviceId);

            if (cal == null && !calDates.Any(x => x.ExceptionType == ExceptionType.Added)) return $"{serviceId} does not exist.";

            if (dateRange == null) dateRange = GetFeedDateRange();
        ...
        {
            FeedInfo info = _reader.Feed.GetFeedInfo();
            ...
            if (_reader.Feed.Calendars.Any())
            {
                minStartLeft = _reader.Feed.Calendars.Min(x => x.StartDate);
                maxEndLeft = _reader.Feed.Calendars.Max(x => x.EndDate);
            }

            if (_reader.Feed.CalendarDates.Any())
            {
                minStartRight = _reader.Feed.CalendarDates.Where(x => x.ExceptionType == ExceptionType.Added).Min(x => x.Date);
                maxEndRight = _reader.Feed.CalendarDates.Where(x => x.ExceptionType == ExceptionType.Added).Max(x => x.Date);
            }
